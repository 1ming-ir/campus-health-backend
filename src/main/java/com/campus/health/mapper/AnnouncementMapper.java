...
public interface AnnouncementMapper {
    @Select("select id, title, type, content, status from announcements where status='PUBLISHED' order by id desc")
    List<Announcement> findPublished();

    @Select("select id, title, type, content, status from announcements order by id desc")
