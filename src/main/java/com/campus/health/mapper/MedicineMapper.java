...
public interface MedicineMapper {
    @Select("select id, name, category, usage_info as usageInfo, caution, prescription_required as prescriptionRequired, status from medicines where status='PUBLISHED' order by id desc")
    List<Medicine> findPublished();

    @Select("select id, name, category, usage_info as usageInfo, caution, prescription_required as prescriptionRequired, status from medicines order by id desc")
