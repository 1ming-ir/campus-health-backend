...
public interface ConsultationRecordMapper {
    ...
    @Select("select id, student_id as studentId, doctor_id as doctorId, symptom, duration, severity, medicine_used as medicineUsed, medicine_name as medicineName, ai_advice as aiAdvice, doctor_reply as doctorReply, status, deleted, archived from consultation_records where student_id=#{studentId} and deleted=false and archived=false order by id desc")
    List<ConsultationRecord> findByStudentId(Long studentId);

    @Select("select c.id, c.student_id as studentId, c.doctor_id as doctorId, u.real_name as studentName, c.symptom, c.duration, c.severity, c.medicine_used as medicineUsed, c.medicine_name as medicineName, c.ai_advice as aiAdvice, c.doctor_reply as doctorReply, c.status, c.created_at as createdAt from consultation_records c left join users u on c.student_id=u.id where c.deleted=false order by c.id desc")
    List<Map<String, Object>> findAllWithStudent();

    @Select("select id, student_id as studentId, doctor_id as doctorId, symptom, duration, severity, medicine_used as medicineUsed, medicine_name as medicineName, ai_advice as aiAdvice, doctor_reply as doctorReply, status, deleted, archived from consultation_records where id=#{id}")
